...
SET msVS_Path="C:\Program Files (x86)\Microsoft Visual Studio\2017\Enterprise\VC\"
...
IF EXIST %msVS_Path% (
	CALL %msVS_Path%\Auxiliary\Build\vcvarsall.bat %currentBuildCompilerOption%
	IF ERRORLEVEL 1 CALL:error 1 "Could not setup compiler for  %PLATFORM%"
	
::IF /I "%currentPlatform%"=="win32" (
	IF NOT "%currentPlatform%"=="%currentPlatform:win32=%" (
		IF NOT "%currentPlatform%"=="%currentPlatform:x64=%" (
			MSBuild %SOLUTIONPATH% /property:Configuration=%CONFIGURATION% /property:Platform=x64 /t:Build /nodeReuse:False
		) ELSE (
			MSBuild %SOLUTIONPATH% /property:Configuration=%CONFIGURATION% /property:Platform=win32 /t:Build /nodeReuse:False
		)
	) ELSE (
		MSBuild %SOLUTIONPATH% /property:Configuration=%CONFIGURATION% /property:Platform=%PLATFORM% /t:Build /nodeReuse:False /m
	)
...
IF NOT "!webRtcLibs!"=="" %msVS_Path%\Tools\MSVC\14.11.25503\bin\Hostx86\x86\lib.exe /OUT:%destinationPath%webrtc.lib !webRtcLibs!
